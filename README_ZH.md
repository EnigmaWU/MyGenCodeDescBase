...
- 细节在这里：[README_Protocol_ZH.md](README_Protocol_ZH.md) | [README_AlgABC_ZH.md](README_AlgABC_ZH.md) | [Protocols/](Protocols/) | [README_UserStories_ZH.md](README_UserStories_ZH.md) | [README_UserGuide_ZH.md](README_UserGuide_ZH.md) | [README_TestGuide_ZH.md](README_TestGuide_ZH.md)
...
- 用集合表达就是：聚合行 =（`startTime..endTime` diff 中的行）∩（`endTime` 仍存活的行）。
- 度量支持**三种模式**，通过阈值参数控制：
...
当前验收基线：AlgC 处理约 **1 GB** 的 `genCodeDescV26.04` 数据，AlgA 用真实 60 天多分支项目验证（见 [README_UserStories_ZH.md](README_UserStories_ZH.md)）。

容量规划用的压力上界模型：**1K 次提交 × 每次 100 个文件 × 每个文件 10K 行增删**，在 `[startTime, endTime]` 窗口内。
...
| **缓解措施** | 并行跑 blame（100 并发）。用 `git blame --incremental` 流式处理。缓存结果。跳过上次以来没变的文件。 | 限制窗口大小。流式处理 diff。按文件路径分片回放。预计算文件改名图。 | 按时间顺序流式读 genCodeDesc——不要一次加载完整输入集（1 GB 基线；200 GB 压力上界）。按文件路径分片存活集。大 JSON 用 mmap。用 SUMMARY 校验条目数。 |
